...
module.exports = (function() {
   ...
   * @param {number} [options.sort] - Sorting object by schema keys ex: {prop1: 1, prop2: -1}, 1 - ascending, -1 - descending.
  ...
  manager.getDatasetData = function(db, filter, projection, options) {
    ...
    return db.allAsync(sqliteTranslation.query, sqliteTranslation.values)
      .then((rows) => {
        ...
        if (_.includes(schema, sqliteConstants.SQLITE_GENERAL_TYPE_OBJECT) || _.includes(schema, sqliteConstants.SQLITE_GENERAL_TYPE_ARRAY)) {
          _.forEach(rows, (row) => {
            const convertedRow = {};
   ...
   * @param {object} update - The update object woith field data to be replaced.
  ...
  manager.updateDataByQuery = function(db, query, update) {
    ...
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        db.get(`SELECT Count(*) AS count FROM ${sqliteConstants.DATABASE_DATA_TABLE_NAME}${whereClause};`, sqliteCountTranslation.values, (error, row) => {
          if (error) reject(error);
          else result = row;
        });
