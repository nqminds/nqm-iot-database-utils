...
async function getDataQuery(db, distinct, filter, projection, options) {
  ...
  if (distinct === false) {
    ...
    if (nqmMeta) result.metaData = await module.exports.getResource(db);
  } else result = [];
 ...
 *     errors.
 */
module.exports.updateData = async function(db, data, upsert, throws) {
...
module.exports.getResource = async function(db, noThrow) {
  ...
  const infoKeys = await sqliteInfoTable.getInfoKeys(db, Object.keys(
    keysToDisplayKeys));
  const resource = {};
