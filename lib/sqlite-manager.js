...
module.exports = (function() {
  ...
  manager.getDatasetData = function(db, filter, projection, options) {
    return manager.getData(db, filter, projection, options);
  };
  ...
  manager.updateData = async function(db, data, upsert, throws) {
    ...
    // promisfy disgusting werid inconsistent callback sqlite3 functions
    // required so that we know when to rollback db if there is a failure
    const runNewStatementAsync = util.promisify( (sqlStatement, callback) => {
      db.run(sqlStatement, callback);
    });
    const prepareStatementAsync = (sqlStatement) => {
      return new Promise( (resolve, reject) => {
        const statement = db.prepare(sqlStatement, (error) => {
          if (error) reject(error);
          else resolve(statement);
        });
      });
    };
    const runStatementAsync = (preparedStatement, params) => {
      return new Promise( (resolve, reject) => {
        preparedStatement.run(params, (error) => {
          if (error) reject(error);
          else resolve();
        });
      });
    ...
    // returns if everything was successful
    // there has got to be a better way to do this then this spaghetti code
    // throws otherwise
    async function promsedResultMaker() {
      // cache all the statements. This is so we can reuse them for speed.
      const cachedStatements = new Map();

      db.serialize();
      try {
        // see https://www.sqlite.org/lang_transaction.html#immediate
        await runNewStatementAsync("BEGIN IMMEDIATE TRANSACTION;");
        for (const dataRow of sqlData) {
          ...
          if (statement === undefined) {
            statement = await prepareStatementAsync(
              insertStatement(uniqueIndex, schema, dataRowKeys, set_upsert)
            );
            cachedStatements.set(dataRowKeys.join(), statement);
          }
          await runStatementAsync(statement, Object.values(dataRow));
        }
        await runNewStatementAsync("COMMIT TRANSACTION;");
      } catch (error) {
        await runNewStatementAsync("ROLLBACK TRANSACTION;");
        throw error;
      } finally {
        db.parallelize();
        for (const preparedStatement in cachedStatements.values()) {
          // finalizing apparently speeds stuff up so lets do that then
          preparedStatement.finalize();
        }
        cachedStatements.clear();
      }
    }
    const promisedResult = promsedResultMaker();
