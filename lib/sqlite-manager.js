...
module.exports = (function() {
   ...
   * @return {module:sqlite-manager~DatasetData}
   */
   ...
   * @return  {Promise<module:sqlite-manager~Resource>}
  ...
  manager.getResource = async function(db, noThrow) {
    ...
    Object.keys(keysToDisplayKeys).forEach( (key) => {
      const display_key = keysToDisplayKeys[key];
      resource[display_key] = defaultValue;
    });
    // map keys from info table to TDX Resource definition.
    info_keys.forEach( (info_key) => {
      Object.keys(info_key).forEach( (key) => {
        resource[keysToDisplayKeys[key]] = info_key[key];
      });
    });
    return resource;
  };
