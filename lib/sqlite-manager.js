...
module.exports = (function() {
  ...
  manager.addData = async function(db, data) {
    ...
    let arrayProcData = sqlData;
    if (ndarrayKeys. length > 0)
      arrayProcData = await sqliteNdarray.writeNdarrayMany(db, data, ndarrayKeys);

    // throws an error if it fails
    await sqliteHelper.executeMany(db, makeSqlStatementStr, arrayProcData);
  ...
  async function getDataQuery(db, distinct, filter, projection, options) {
    ...
    // Save the ndarray data to file
    let arrayProcData = rows;
    if (ndarrayKeys.length > 0 && _.includes(ndarrayKeys, sqliteConstants.SQLITE_GENERAL_TYPE_OBJECT)) {
      console.log(rows);
      arrayProcData = await sqliteNdarray.readNdarrayMany(db, rows, ndarrayKeys);
    }
