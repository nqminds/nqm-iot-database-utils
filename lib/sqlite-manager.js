...
module.exports = (function() {
  ...
  manager.addData = async function(db, data) {
    ...
    const newData = await sqliteNdarray.writeNdarrayMany(db, sqlData, ndarrayKeys);

    // throws an error if it fails
    await sqliteHelper.executeMany(db, makeSqlStatementStr, newData);
  ...
  async function getDataQuery(db, distinct, filter, projection, options) {
    ...
    const rows = await db.allAsync(
      sqliteTranslation.query, sqliteTranslation.values)
