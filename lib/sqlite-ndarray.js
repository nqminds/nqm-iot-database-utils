    ...
    return new Promise((resolve, reject) => {
      ...
      for (const dataRow of data) {
        ...
        for (const keyValue of dataKeys) {
          ...
          meta["s"] = meta["s"] || [0];
          const dataBuffer = ndamodule.getTypedArray(meta["t"], meta["s"]);
          ...
          try {
            ...
            const bytesRead = fs.readSync(fd, dataBuffer, 0, Buffer.byteLength(dataBuffer), 0);
            fs.closeSync(fd);
          
            // Check if what's read is consistent with metadata
            if (bytesRead !== dataFileStats.size)
              reject(Error(`Metadata size different to file size for ${JSON.stringify(meta)}`));
          } catch (error) {
            reject(error);
          }
          // Assign the new ndarray object
          newRow[keyValue] = nd(dataBuffer, meta["s"], majorToStride(meta["c"], meta["s"]));
        }
