...
module.exports = (function() {
  ...
  ndamodule.getTypedArray = function(numpyType, numpySize) {
    const size = numpySize.reduce((a, b) => {
      return a * b;
    });
   ...
   * @param {string|string[]} key - The key(s) representing the ndarray(s).
   * @returns {array<object>} - The modified data list with ndarray objects
   */
  ndamodule.readNdarrayMany = function(db, data, key) {
    ...
    for (const dataRow of data) {
      ...
      for (const keyValue of dataKeys) {
        ...
        meta["s"] = meta["s"] || [0];
        const buffer = ndamodule.getTypedArray(meta["t"], meta["s"]);

        try {
          const filePath = path.join(db.dataFolder, meta["p"]);
          const fd = fs.openSync(filePath, "r");
          const bytesRead = fs.readSync(fd, buffer, 0, Buffer.byteLength(buffer), 0);
          fs.closeSync(fd);
        } catch (error) {
          throw error;
        }
        // Assign the new ndarray object
        newRow[keyValue] = nd(buffer, meta["s"], majorToStride(meta["c"], meta["s"]));
      }
